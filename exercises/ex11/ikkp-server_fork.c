...
{
    // Causes segfault. Comment the lines below to let the program run. -------------------------
    struct sigaction sa;
    ...
    sa.sa_sigaction = error;

    sigaction(SIGSEGV, &sa, NULL);
    printf("Segmentation fault. Forked process terminated.\n");
    return -1;
    // ------------------------------------------------------------------------------------------
        ...
        if(!fork()){
          ...
          if (say(connect_d, "Surrealist giraffe.\n") == -1) {
              ...
              if (say(connect_d, "Bathtub full of brightly-colored machine tools.\n") == -1) {
                  // comment this out to test with multiple connections
                  close(connect_d);
                  continue;
              }
          }
