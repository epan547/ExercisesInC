...
likely to be confused with `glibc`? <br>

Glib is a group of low-level system libraries written in C and developed mainly by GNOME. It provides features like advanced data structures, caches to provide memory management, standard macros, warnings, and dynamic loading of modules.
...
to another library, or just using core C features? <br>

GLib's behavior is well-defined on any platform that GTK+ supports, as opposed to the native functions which may perhaps sometimes partway work.
Thus, it is more portable across different systems, and makes many functions, like free, much easier.

*  What do you have to do to compile and run a "Hello GLib" example? <br>

compile: cc hello.c `pkg-config --cflags --libs glib-2.0` -o hello
run: ./hello
